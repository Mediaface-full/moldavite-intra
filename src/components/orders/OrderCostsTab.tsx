...
const TYPE_KEYS: Array<{ key: string; label: string; icon: IconName }> = [
  { key: 'transport', label: 'Doprava',     icon: 'truck' },
  { key: 'photo',     label: 'Focení',      icon: 'camera' },
  { key: 'packing',   label: 'Balení',      icon: 'package' },
  { key: 'storage',   label: 'Skladování',  icon: 'storage' },
  { key: 'fees',      label: 'Poplatky',    icon: 'cash' },
  { key: 'customs',   label: 'Clo',         icon: 'cart' },
  { key: 'work',      label: 'Práce / čas', icon: 'tools' },
  { key: 'other',     label: 'Jiné',        icon: 'tag' },
];
                ...
                editId === c.id ? (
                  ...
                  <tr key={c.id} className="border-b border-border last:border-0 hover:bg-muted/40 transition-colors">
                    <td className="px-4 py-3 font-mono text-xs uppercase tracking-wider text-muted-foreground">
                      {(() => {
                        const t = TYPE_KEYS.find((t) => t.key === c.typeKey);
                        return (
                          <span className="inline-flex items-center gap-1.5">
                            {t && <Icon name={t.icon} className="w-3.5 h-3.5" />}
                            {t?.label ?? c.typeKey}
                          </span>
                        );
                      })()}
